...
- 发布确认日期：2026-08-16
- 发布方式：`deploy/upgrade.sh` 完整升级
- 已部署提交：`47f9e2a94eac92b01e52cfe84c6cda112e381a12`
- 发布版本：`v0.3.0`
- 部署前加密数据库备份：`db-deployment-20260816T162607Z-59.dump.enc`
- 数据库迁移：本轮无新增迁移；`v0.3.0` 已包含此前迁移

| 检查项 | 结论 |
|---|---|
| 提交前质量门槛 | 通过；`ruff check .`、`pytest` 全量 `357 passed`、Django check 与迁移检查通过 |
| `deploy/upgrade.sh` | 通过 |
| 数据库迁移 | 通过；无新增迁移 |
| `deploy/verify-deployment.sh` | 通过 |
| 财务与主题完整性 | 通过 |
