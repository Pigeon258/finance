...
本文档定义 Personal Finance 的技术架构、模块边界、数据模型、账务计算规则、核心业务流程、账单导入机制、视觉主题系统、安全设计、备份恢复方案和云端部署方式。

本文档用于说明当前实现采用的架构与必须保持的系统不变量。
...
视觉主题支持全界面覆盖、沉浸式背景、组件换肤、动态状态和本地主题管理。Personal Finance 不使用页面注入，也不依赖运行时猜测 DOM；应用直接维护自己的模板和组件契约。
...
具体白名单由主题运行时实现固定，并以负向安全测试证明失败关闭。
...
如需记录多个已安装包的来源、校验结果和版本，应按实际使用需要引入最小 `InstalledTheme` 模型，不建立可执行插件模型。
