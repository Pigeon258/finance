...
- 支付宝/微信 CSV、XLSX、单层 ZIP 的安全解析与待确认隔离区。

当前版本已经可以在本地试用上述核心流程和账单解析，但导入确认入账、导出与加密备份、Docker Compose 生产部署尚未实现。请勿把当前版本直接作为生产服务暴露到公网。
...
8. 在“账单导入”中上传虚构的支付宝或微信账单，检查解析结果是否进入待确认隔离区；当前不会创建正式交易。
