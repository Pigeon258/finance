...
- 信用卡账期、全额还款与退款抵扣；
- 商品分期、提前结清和分期退款；
- 月度预算、可批量设置的分类预算、储备资金、固定支出与预计收入；
...
2026-08-15 完成了五轮生产维护升级：页面状态中文化与组件统一、交易表单辨识与全局间距优化、标签按收入/支出类型隔离、标签管理页面、分类预算批量编辑与计划分类过滤。生产运行时提交为 `5a076c8`，数据库已应用 `ledger.0006_tag_applies_to`。逐次提交、部署备份和验收结论见：
...
5. 在“预算管理”中设置月度总预算，并通过“设置分类预算”为餐饮、日用品等分类填写明细预算。
...
2026-07-13 已在 Ubuntu 22.04 / WSL 2 对提交 `b91668c` 完成本地容器验收；2026-08-03 已完成目标服务器首次生产上线验收并固化 `v0.1.0`；2026-08-05 已完成视觉主题系统生产升级并固化 `v0.2.0`。2026-08-15 已完成页面状态、交易表单、标签类型隔离、标签管理和分类预算编辑五轮生产维护升级，详细证据见 `docs/acceptance.md`，汇总见 `docs/maintenance-history.md`。生产环境隔离恢复和整机重启演练继续按运维计划执行。
