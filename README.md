...
当前版本已经可以在本地试用上述核心流程、账单导入和加密业务备份恢复，但 Docker Compose 生产部署、数据库自动备份及轮换尚未实现。请勿把当前版本直接作为生产服务暴露到公网。
...
建议仅使用虚构测试数据。业务备份文件默认保存在下载目录，恢复前自动备份保存在 `BUSINESS_BACKUP_DIR`（默认项目 `backups/`，已被 Git 忽略）。任务 15 完成前，自动数据库备份和备份轮换仍需手工处理。
