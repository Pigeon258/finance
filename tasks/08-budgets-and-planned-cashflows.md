...
- 创建启用的计划现金流时，先生成从开始日期起 12 个自然月窗口内的期次；用户可通过显式“生成至”操作延长窗口。
- 期次按 `(plan, due_date)` 幂等生成；已经确认、跳过或过期的日期不得重新生成。
- 每月 29～31 日及闰日遇到目标月份或年份不存在该日时取月末；不使用上一次收敛后的日期推导下一期。
- 停用计划只阻止生成新期次，保留既有期次及其审计状态。
